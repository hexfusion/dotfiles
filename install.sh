...
link_config "$DOTFILES_DIR/shared/zellij/config.kdl" "$CONFIG_DIR/zellij/config.kdl"
mkdir -p "$CONFIG_DIR/zellij/layouts"
link_config "$DOTFILES_DIR/shared/zellij/layouts/design.kdl" "$CONFIG_DIR/zellij/layouts/design.kdl"
link_config "$DOTFILES_DIR/shared/broot/conf.hjson" "$CONFIG_DIR/broot/conf.hjson"
link_config "$DOTFILES_DIR/shared/broot/verbs.hjson" "$CONFIG_DIR/broot/verbs.hjson"
link_config "$DOTFILES_DIR/shared/lazygit/config.yml" "$CONFIG_DIR/lazygit/config.yml"
link_config "$DOTFILES_DIR/shared/vimrc" "$HOME/.vimrc"
...
# Shell profile.d
info "Installing shell profile.d..."
link_config "$DOTFILES_DIR/shared/shell/profile.d" "$CONFIG_DIR/shell/profile.d"

info "Checking bashrc for profile.d loader..."
if ! grep -q 'shell/profile.d' "$HOME/.bashrc" 2>/dev/null; then
    cat >> "$HOME/.bashrc" << 'BASHEOF'

# hexfusion dotfiles
for f in ~/.config/shell/profile.d/*.sh; do [ -r "$f" ] && source "$f"; done
BASHEOF
    success "Added profile.d loader to ~/.bashrc"
else
    success "Profile.d loader already in ~/.bashrc"
fi

# Install zellij if not present
info "Checking for zellij..."
if ! command -v zellij &>/dev/null && [ ! -x "$LOCAL_BIN/zellij" ]; then
    info "Installing zellij to $LOCAL_BIN..."
    curl -sL https://github.com/zellij-org/zellij/releases/latest/download/zellij-x86_64-unknown-linux-musl.tar.gz | tar xz -C "$LOCAL_BIN"
...
info "Checking for vim-fugitive..."
FUGITIVE_DIR="$HOME/.vim/pack/plugins/start/vim-fugitive"
if [ ! -d "$FUGITIVE_DIR" ]; then
    mkdir -p "$HOME/.vim/pack/plugins/start"
    git clone --depth 1 https://github.com/tpope/vim-fugitive.git "$FUGITIVE_DIR"
    success "vim-fugitive installed"
else
    success "vim-fugitive already installed"
fi

# Install broot
info "Checking for broot..."
if ! command -v broot &>/dev/null && [ ! -x "$LOCAL_BIN/broot" ]; then
    info "Installing broot to $LOCAL_BIN..."
    curl -Lo "$LOCAL_BIN/broot" "https://dystroy.org/broot/download/x86_64-linux/broot"
...
echo "  - Launch zellij workspace: Super+z or 'zd'"
echo "  - Start/resume Claude in a tab: 'c'"
